...
import { ISubscriptionRepository, SUBSCRIPTION_REPOSITORY } from '@/subscription/infrastructure/repositories/subscription.repository.interface';
...
describe('SubscriptionService', () => {
    ...
    describe('subscribe', () => {
        it('should throw ConflictException if already subscribed', async () => {
            ...
            const dto: CreateSubscriptionDto = { email: 'a@b.com', city: 'City', frequency: UpdateFrequency.DAILY };
        ...
        it('should create subscription and send confirmation email', async () => {
            ...
            const dto: CreateSubscriptionDto = { email: 'a@b.com', city: 'City', frequency: UpdateFrequency.HOURLY };
    ...
    describe('sendBatch', () => {
        it('should fetch weather and send emails for daily frequency', async () => {
            ...
            (weatherService.getCurrent as jest.Mock).mockResolvedValue({ temperature: 10, humidity: 50, description: 'Desc' });
        ...
        it('should use hourly template for hourly frequency', async () => {
            ...
            (repo.findConfirmedByFrequency as jest.Mock).mockResolvedValue(subs);
            (weatherService.getCurrent as jest.Mock).mockResolvedValue({ temperature: 5, humidity: 80, description: 'Desc' });
